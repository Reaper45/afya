...
const ButtonWrapper = styled("button")<{ active?: boolean; iconPosition?: "left" | "right" }>`
  outline: none;
  ...
  `}
  > span {
    margin-${props => props.iconPosition === "right" ? "left" : "right"}: 1em;
    fill: ${props => props.active ? props.theme.colors.primary : props.theme.colors.grey};
  }
`;
...
const Button: React.FC<ButtonHTMLAttributes<HTMLButtonElement> & { iconPosition?: "left" | "right", icon?: JSX.Element; active?: boolean; }> = ({ children, icon, ...props}) => {
  return (
    <ButtonWrapper {...props} >
